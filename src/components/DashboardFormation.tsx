...
export function DashboardFormation({ resume }: { resume: ResumeFormation }) {
    ...
    return (
        <Link
            href="/formation"
            className="block bg-white/15 backdrop-blur-sm rounded-2xl p-4 border border-white/20 active:scale-[0.99] transition-transform"
        >
            ...
            {!complet && resume.prochain && (
                <p className="text-[11px] text-white/50 mt-4 truncate">
            ...
            {/* Le CTA reste transparent, cohérent avec le reste de la carte — le blanc
                ...
                sur ce dégradé. */}
            <div className="flex items-center justify-center gap-2 bg-white/15 border border-white/20 rounded-xl py-3 mt-2.5 text-[13px] font-black text-white">
                {libelleCta}
