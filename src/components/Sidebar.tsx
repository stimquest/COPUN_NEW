...
/** Les deux pôles de l'application, chacun sous son titre, puis les outils transverses. */
export function Sidebar({ role, fullName, email, clubName, formationEnCours }: SidebarProps) {
    ...
    return <aside className="co-sidebar">
        ...
        <nav aria-label="Navigation principale">
            <Link href="/stages" aria-current={activeNavigation(path, '/stages') ? 'page' : undefined}><House size={20}/>Accueil</Link>
            {NAV_POLES.map(pole => <div key={pole.titre} className="co-sidebar-pole">
                <p>{pole.titre}</p>
                {pole.liens.map(item => {
                    const Icon = item.icon;
                    return <Link key={item.href} href={item.href} aria-current={activeNavigation(path, item.href) ? 'page' : undefined}>
                        <Icon size={20}/>{item.name}{item.href === '/formation' && formationEnCours && <i className="co-nav-dot"/>}
                    </Link>;
                })}
            </div>)}
        </nav>
